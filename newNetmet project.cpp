...
public:
    ...
    void removeUser(const string &username) {
        if (adjList.find(username) != adjList.end()) {
            ...
            for (auto &user : adjList) {
                auto &friends = user.second;
                friends.erase(remove(friends.begin(), friends.end(), username), friends.end());
            }

            cout << "\n---------------------------------------------------\n";
            cout << username << " removed from NetMet.\n";
            cout << "---------------------------------------------------\n";
        } else {
    ...
    void recommendFriends(const string &username) {
        if (adjList.find(username) == adjList.end()) {
            cout << username << " does not exist in the network.\n";
            return;
        }

        set<string> recommendations;
        unordered_set<string> directFriends(adjList[username].begin(), adjList[username].end());

        // For each friend of the user
        for (const string &friendUser : directFriends) {
            // Check the friends of this friend
            for (const string &fof : adjList[friendUser]) {
                // Recommend if they are not the user themselves and are not already a direct friend
    ...
    void menu() {
        ...
        while (true) {
            ...
            cout << "6. Save Network to File\n";
            cout << "7. Load Network from File\n";
            cout << "8. Find Mutual Friends\n";
            cout << "9. Recommend Friends\n";  // New menu option
            cout << "10. Exit\n";
            cout<<"===========================================\n";
            ...
            if (userInput == "1") {
                ...
                // Recommend friends
                string username;
                cout << "Enter username for friend recommendations: ";
                cin >> username;
                recommendFriends(username);  // Call the new function
